...
pub fn generate_gdtf_schema() -> anyhow::Result<()> {
    let mut config = default_config();
    configure_parser(&mut config, "codegen/schema/gdtf.xsd");
    ...
    rename_schema_content(&mut schemas, &[("GDTF", "GdtfDescription")]);
    ...
    std::fs::write("mvr-gdtf/src/gdtf/schema.rs", code)?;
